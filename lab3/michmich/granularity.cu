...
{
    __shared__ float ds_A[TILE_WIDTH][TILE_WIDTH], ds_B[TILE_WIDTH][2*TILE_WIDTH];
    float sum1 = 0;
    float sum2 = 0;
    ...
    for(int ph=0; ph<ceil((float)width_A/TILE_WIDTH); ph++){
        ...
        int sid_B = (ph*TILE_WIDTH +threadIdx.x)*width_B + ((TILE_WIDTH * 2 * blockIdx.y) + threadIdx.y);
        if((TILE_WIDTH * 2 * blockIdx.y + threadIdx.y) < width_B && (TILE_WIDTH * ph + threadIdx.x) < width_A){
            ds_B[threadIdx.x][threadIdx.y] = B[sid_B];
        }else{
            ds_B[threadIdx.x][threadIdx.y] = 0;
        }

        int sid_B2 = (ph*TILE_WIDTH +threadIdx.x)*width_B + ((TILE_WIDTH * (2 * blockIdx.y + 1)) + threadIdx.y);
        if(((2 * blockIdx.y + 1)*TILE_WIDTH + threadIdx.y) < width_B && (TILE_WIDTH * ph + threadIdx.x) < width_A){
            ds_B[threadIdx.x][threadIdx.y + TILE_WIDTH] = B[sid_B2];
        }else{
            ds_B[threadIdx.x][threadIdx.y + TILE_WIDTH] = 0;
        }
        __syncthreads();
        for(int i = 0; i<TILE_WIDTH; i++){
            
            sum1+= ds_A[threadIdx.x][i] * ds_B[i][threadIdx.y];
        }
        for(int i = 0; i<TILE_WIDTH; i++){
            
            sum2+= ds_A[threadIdx.x][i] * ds_B[i][threadIdx.y+TILE_WIDTH];
        }
        __syncthreads();
    }
    int idx_x = threadIdx.x + blockIdx.x * blockDim.x;
    int idx_y = threadIdx.y + (blockIdx.y*2) * blockDim.y;
    if(idx_x < height_A && idx_y < width_B){
        C[idx_x * width_B + idx_y ] = sum1;
    }
    idx_y = threadIdx.y + (blockIdx.y*2 + 1) * blockDim.y;
    if(idx_x < height_A && idx_y < width_B){
        C[idx_x * width_B + idx_y ] = sum2;
    }
}
//
...
int main(int argc, char **argv)
{
    ...
    dim3 dimGrid(ceil((float)heightA/ TILE_WIDTH), ceil((float)widthB/ (TILE_WIDTH*2)));
    ...
    if (true)
    {
        FILE *fp = fopen("granularity_out.txt", "w");
        if (fp == NULL)
        {
